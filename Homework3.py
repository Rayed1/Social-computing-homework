...
def moderate_content(content):
    ...
    moderated_content = re.sub(URL_PATTERN, '[link removed]', original_content)

    #Rule 1.2.3
    alphabetic_chars = [char for char in moderated_content if char.isalpha()]
    if len(alphabetic_chars) > 15:
        uppercase_count = sum(1 for char in alphabetic_chars if char.isupper())
        uppercase_percentage = uppercase_count / len(alphabetic_chars)
        if uppercase_percentage > 0.70:
            score = 0.5
    
    #My rule
    MENTION_PATTERN = r'@\w+'
    mention_matches = re.findall(MENTION_PATTERN, moderated_content)
    if len(mention_matches) > 5:
        extra_mentions = len(mention_matches) - 5
        score = extra_mentions * 1
...
def user_risk_analysis(user_id):
    ...
    for post in user_posts:
        ...
        post_score_total += post_score
    
    if user_posts:
        average_post_score = post_score_total / len(user_posts)
    else:
    ...
    for comment in user_comments:
        ...
        comment_score_total += comment_score
    
    if user_comments:
        average_comment_score = comment_score_total / len(user_comments)
    else:
    ...
    if isinstance(user_created_at, str):
        ...
        user_created_dt = user_created_at
    
    account_age_days = (datetime.now() - user_created_dt).days
    ...
    #My risk
    #Users posting more than 10 times per day are suspicious 
    if account_age_days > 0:
        total_posts = len(user_posts) if user_posts else 0
        total_comments = len(user_comments) if user_comments else 0
        total_activity = total_posts + total_comments
        activity_per_day = total_activity / account_age_days
        
        if activity_per_day > 10:
            user_risk_score += 1.0
...
def recommend(user_id, filter_following):
    ...
    # Step 1: Get posts the user reacted to positively
    ...
    stop_words = {'a', 'an', 'the', 'in', 'on', 'is', 'it', 'to', 'for', 'of', 'and', 'with', 'this', 'that'}
